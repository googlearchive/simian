...
class Auth1(AuthBase):
  ...
  def __init__(self, *args, **kwargs):
    ...
    self._key = None
    self._cert = None
    self._ca_pem = CA_PUBLIC_CERT_PEM
    self._server_cert_pem = SERVER_PUBLIC_CERT_PEM
  ...
  def _PatchTlslite(self, public_key, hasattr_=hasattr):
    ...
    def _new_rawPublicKeyOp(self, c):
      ...
      Changes:
       1) rewritten to work from this namespace
       2) support input data sizes in 's' of 1 or 2 bytes too small
    ...
    public_key._rawPublicKeyOp = types.MethodType(
        _new_rawPublicKeyOp, public_key)
    public_key._patch_auth1 = 1
  ...
  def VerifyCertSignedByCA(self, cert):
    ...
    try:
      # invoke the patch all the time.  tlslite decodes the long signature
      # back into an int and drops the padding which is correctly placed
      # into most x509 certificates generated by openssl tools, so
      # guessing based on the signature length here won't help us.  it
      # is always going to be a perfect RSA 'n' bits.
      pk = ca_cert.GetPublicKey()
      self._PatchTlslite(pk)
  ...
  def VerifyDataSignedWithCert(self, data, signature, cert=None):
    ...
    try:
      ...
      modulus_len = tlslite.utils.cryptomath.numBytes(pk.n)
      # we can predict whether the signature is lacking padding here because
      # our own client using flawed tlslite libraries wrote it.  if tlslite
      # is replaced on the client, PatchTlslite might need to go fulltime by
      # removing the following conditional:
      if len(signature_b) < modulus_len:
        self._PatchTlslite(pk)
